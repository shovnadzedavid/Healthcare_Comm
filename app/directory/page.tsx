                ...
                <div className="space-y-4">
                  ...
                  {u.workplace ? (
                    <div className="p-3 bg-slate-50 dark:bg-navy-950/70 rounded-2xl border border-slate-100 dark:border-slate-800/60 flex items-start gap-2 text-xs text-slate-600 dark:text-slate-300">
                      ...
                      <span className="line-clamp-2 leading-relaxed">{u.workplace}</span>
                    </div>
                  ) : (
                    <div className="p-3 bg-slate-50 dark:bg-navy-950/30 rounded-2xl border border-transparent flex items-center gap-2 text-xs text-slate-400 italic">
                      <Briefcase className="w-3.5 h-3.5 shrink-0 opacity-50" />
                      <span>ორგანიზაცია მითითებული არ არის</span>
                    </div>
                  )}
                </div>

                <div className="pt-5 mt-5 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between gap-2.5">
                  <Link
                    href={`/messages?user=${u.id}`}
                    className="flex-1 py-2.5 px-3.5 rounded-2xl text-xs font-bold text-slate-700 dark:text-slate-200 bg-slate-100 dark:bg-navy-800 hover:bg-cyan-500/10 hover:text-cyan-600 dark:hover:text-cyan-400 border border-slate-200 dark:border-slate-700/80 transition-all flex items-center justify-center gap-2"
                  >
                  ...
                  {isAdded ? (
                    <button
                      ...
                      disabled={isActing}
                      className="p-2.5 rounded-2xl text-slate-400 hover:text-rose-500 hover:bg-rose-500/10 border border-slate-200 dark:border-slate-800 transition-colors cursor-pointer"
                    ...
                    <button
                      ...
                      disabled={isActing}
                      className="flex-1 py-2.5 px-3.5 rounded-2xl text-xs font-bold text-white bg-gradient-to-r from-cyan-600 to-teal-500 hover:opacity-95 shadow-md shadow-cyan-500/20 flex items-center justify-center gap-2 transition-all disabled:opacity-50 cursor-pointer"
                    >
                      {isActing ? (
