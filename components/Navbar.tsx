...
export default function Navbar() {
  ...
  const navItems = [
    ...
    { name: 'დისკუსიები & პოლიტიკა', href: '/discussions', icon: MessageSquare },
    { name: 'ბლოგი & ანალიტიკა', href: '/blog', icon: BookOpen },
    { name: 'ჩატი', href: '/messages', icon: Send },
  ];

  return (
    <nav className="sticky top-0 z-40 w-full border-b border-slate-200/80 dark:border-slate-800/80 bg-white/90 dark:bg-navy-950/90 backdrop-blur-xl shadow-xs transition-colors">
          ...
          <Link href="/" className="flex items-center gap-3.5 group">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[10px] uppercase tracking-widest text-slate-400 dark:text-slate-400 font-bold">
                Health Policy & Academic Society
              </span>
            </div>
          </Link>
              ...
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    key={item.href}
                    href={item.href}
                    className={`flex items-center gap-2 px-4 py-2.5 rounded-2xl text-[14px] font-extrabold transition-all ${
                      isActive
                        ? 'bg-cyan-500/15 text-cyan-700 dark:text-cyan-400 shadow-xs'
                        ...
                        notifications.map((n) => (
                          <div
                            ...
                            className={`p-3 rounded-xl transition-colors cursor-pointer ${
                              !n.is_read ? 'bg-cyan-500/10 font-semibold text-slate-900 dark:text-white' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-navy-950'
                            }`}
                          >
            ...
            {user ? (
              <>
                <Link
                  ...
                  className="hidden sm:inline-flex items-center gap-2 bg-gradient-to-r from-cyan-600 to-teal-500 hover:opacity-95 text-white px-5 py-2.5 rounded-2xl text-xs sm:text-sm font-extrabold shadow-md shadow-cyan-500/20 active:scale-95 transition-all"
                >
                ...
                <Link
                  href="/profile"
                  className={`hidden sm:inline-flex items-center gap-2 px-4 py-2.5 rounded-2xl text-xs sm:text-sm font-bold border ${
                    pathname === '/profile'
